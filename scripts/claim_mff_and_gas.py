#!/usr/bin/env python3
"""
Warning: This script does not currently work. After claiming the initial MFF, this script is unable
to send the MFF to the same wallet/address (used to work early February 2018). Possibly
due to some neo-python changes. Now added `claim_mff_and_gas_fixedwallet.py` which uses
a fixed wallet/address.

---

With this script you can transfer all MFF from the default contract on a private network
such as the one created by the mfc-privatenet-docker image, without having to
use the mfc-gui client (or Windows at all). It also waits a little bit to generate
GAS and claims it.

The output of the script is two things:

* A wallet with 100,000,000 MFF and 48 GAS (after 1 minute) which you can use with prompt.py
* A WIF private key you can use with any client.

Run it like this:

    python3 scripts/claim_mff_and_gas.py

There are several parameters you can configure with cli args. Take a look at the help:

    python3 scripts/claim_mff_and_gas.py -h
...
# default multi-sig contract from the mfc-privatenet-docker image. If you are using a different private net
...
class PrivnetClaimall(object):
    ...
    def claim_initial_neo(self, target_address):
        ...
        for pkey, wif in nodekeys.items():
            ...
            if tx_json == 'success':
                print("Finished, {} should now own all the MFF on the private network.".format(target_address))
                break
            i += 1
